...
func (s *Service) generateSummary(ctx context.Context, videoID string, llmConfig *store.LLMConfig) (string, string, error) {
	...
	// TODO: Fetch and parse actual subtitle content from the URL
	// For now, we'll use a placeholder subtitle text
	subtitleText := "This is placeholder subtitle text. In a real implementation, we would fetch the actual subtitles from the URL stored in entry.AutoSubtitles"
